...
static QueueHandle_t __ble_queue = NULL;

#define GATTS_TABLE_TAG "BT"

#define NPSR_CAR_PROFILE_NUM 1
#define CAR_REMOTE_PROFILE_APP_IDX 0
#define NPSR_CAR_APP_ID 0x55
#define NPSR_CAR_DEVICE_NAME "NpsrCar"
#define NPSR_CAR_SVC_INST_ID 0
...
static uint16_t car_remote_handle_table[NPSR_CAR_NB];

static uint8_t test_manufacturer[4] = {'N', 'P', 'S', 'R'};
...
// config adv data
static esp_ble_adv_data_t car_adv_config = {
  .set_scan_rsp = false,
...
static esp_ble_adv_data_t car_scan_rsp_config = {
  .set_scan_rsp = true,
  .include_name = true,
...
static esp_ble_adv_params_t npsr_car_adv_params = {
  .adv_int_min = 0x100,
  .adv_int_max = 0x100,
...
static struct gatts_profile_inst car_profile_tab[NPSR_CAR_PROFILE_NUM] = {
  [CAR_REMOTE_PROFILE_APP_IDX] = {
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
  ...
  case ESP_GAP_BLE_SCAN_RSP_DATA_SET_COMPLETE_EVT:
    ...
    {
      esp_ble_gap_start_advertising(&npsr_car_adv_params);
    }
    break;
  ...
  case ESP_GAP_BLE_PASSKEY_REQ_EVT: /* passkey request event */
    ...
    //esp_ble_passkey_reply(car_profile_tab[CAR_REMOTE_PROFILE_APP_IDX].remote_bda, true, 0x00);
    break;
  ...
  case ESP_GAP_BLE_SET_LOCAL_PRIVACY_COMPLETE_EVT:
    ...
    esp_err_t ret = esp_ble_gap_config_adv_data(&car_adv_config);
    if (ret)
    {
    ...
    ret = esp_ble_gap_config_adv_data(&car_scan_rsp_config);
    if (ret)
    {
      ESP_LOGE(GATTS_TABLE_TAG, "config adv data failed, error code = %x", ret);
    }
                                        ...
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
  ...
  case ESP_GATTS_WRITE_EVT:
    ...
    // single ascii to number
    uint8_t single_byte = (uint8_t)param->write.value[0] - 0x30;
    xQueueSend(__ble_queue, &single_byte, pdMS_TO_TICKS(50));
  ...
  case ESP_GATTS_CREAT_ATTR_TAB_EVT:
  {
    ...
    {
      if (param->add_attr_tab.num_handle == NPSR_CAR_NB)
      {
        memcpy(car_remote_handle_table, param->add_attr_tab.handles,
               sizeof(car_remote_handle_table));
        esp_ble_gatts_start_service(car_remote_handle_table[NPSR_CAR_SVC]);
      }
      else
      {
        ESP_LOGE(GATTS_TABLE_TAG, "Create attribute table abnormally, num_handle (%d) doesn't equal to NPSR_CAR_NB(%d)",
                 param->add_attr_tab.num_handle, NPSR_CAR_NB);
      }
    }
    else
    {
                                ...
                                esp_ble_gatts_cb_param_t *param)
{
  ...
  if (event == ESP_GATTS_REG_EVT)
  {
    ...
    {
      car_profile_tab[CAR_REMOTE_PROFILE_APP_IDX].gatts_if = gatts_if;
    }
    else
    {
  ...
  {
    int idx;
    for (idx = 0; idx < NPSR_CAR_PROFILE_NUM; idx++)
    {
      if (gatts_if == ESP_GATT_IF_NONE || /* ESP_GATT_IF_NONE, not specify a certain gatt_if, need to call every profile cb function */
          gatts_if == car_profile_tab[idx].gatts_if)
      {
        if (car_profile_tab[idx].gatts_cb)
        {
          car_profile_tab[idx].gatts_cb(event, gatts_if, param);
        }
      }
    }
  } while (0);
...
esp_err_t npsr_bluetooth_le_init()
{
  ...
  ret = esp_ble_gatts_app_register(NPSR_CAR_APP_ID);
  if (ret)
  {
  ...
  /* Create a queue capable of containing 10 single-byte values. */
  __ble_queue = xQueueCreate(10, sizeof(uint8_t));
  if (__ble_queue == NULL)
  {
    ESP_LOGE(GATTS_TABLE_TAG, "xQueueCreate failed");
    return -1;
  }
